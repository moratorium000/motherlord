...
**규칙 P5-a. 개념 소유권(Concept Ownership) 원칙** — 모든 주요 개념은 **정확히 하나의 모듈만** 이를 "소유"하여 정식 정의한다. 다른 모듈은 정의를 반복하지 않고 **`→ M0X §Y 참조`** 로 링크만 건다. 소유권 표는 §6.1.
...
flowchart TD
    ...
    P1["<b>Part I. RF의 공용어</b><br/>M01 · M02 · M03<br/><i>dB · 전송선로 · S파라미터/스미스 차트</i>"]
...
| 주차 | 모듈 | 본문 분량 | 배정 시간 | 산출물 |
|---|---|---|---|---|
| 1 | M00 오리엔테이션 | 6 h | 8 h | 학습 계약서 · 장비 조달 계획 |
| 2 | M01 데시벨과 전력 | 8 h | 8 h | dB 변환 계산 워크시트 |
| 3–4 | M02 전송선로 | 12 h | 16 h | 마이크로스트립 임피던스 계산 결과 |
| 5–6 | M03 S-파라미터·스미스 차트 | 12 h | 16 h | Touchstone 파일 해석 리포트 |
| 7 | M04 RF 실험실 입문 | 8 h | 8 h | 커넥터 점검 체크리스트 |
| 8 | M05 첫 측정 | 10 h | 8 h | S11/스펙트럼 측정 리포트 #1 |
| 9 | M06 수동소자와 공진 | 8 h | 8 h | 인덕터 SRF 실측 |
| 10 | M07 필터와 수동 네트워크 | 9 h | 8 h | 필터 특성 측정·규격 대조 |
| 11–12 | M08 증폭기 | 12 h | 16 h | 이득/NF/P1dB/IP3 개념 문제집 |
| 13–14 | M09 주파수 변환과 신호원 | 12 h | 16 h | 주파수 계획서(스퍼 표) |
| 15 | M10 안테나와 전파 | 10 h | 8 h | 링크 버짓 스프레드시트 |
| 16 | M11 트랜시버 아키텍처 | 11 h | 8 h | 아키텍처 비교 트레이드오프 표 |
| 17–18 | M12 시스템 예산 설계 | 14 h | 16 h | **캐스케이드 예산표** |
| 19 | M13 변조와 신호품질 | 12 h | 8 h | EVM/ACLR 규격 해석서 |
| 20 | M14 정밀측정 I — VNA 교정 | 13 h | 8 h | 교정 검증 리포트 |
| 21 | M15 정밀측정 II — NF/위상잡음/변조 | 14 h | 8 h | 4종 측정 리포트 |
| 22 | M16 DUT 스펙 검증·튜닝·자동화 | 14 h | 8 h | **테스트 플랜 + 자동화 스크립트** |
| 23 | M17 RF 보드 설계와 인증 | 16 h | 8 h | 보드 설계 리뷰 패키지 |
| 24 | Capstone (P1 시스템 설계) | — | 8 h | RX/TX 예산표 · 주파수 계획서 |
| **25–27** | **Capstone (P2–P4)** | — | 24 h | **보드 설계 · 측정 · 튜닝 · 판정 · 리뷰** |

> **주차와 분량의 셈**: 모듈 18편의 본문 분량 합계는 **201 h**인데, 위 표가
> 모듈에 배정한 것은 **23주 × 8 h = 184 h**입니다. **17 h가 모자랍니다.**
> 표를 예쁘게 맞추려고 분량 쪽 숫자를 줄이면 실제로 해 보는 사람이
> 시간에 쫓기므로, 모자란다는 사실을 그대로 적고 닫는 방법을 둡니다.
>
> | 닫는 방법 | 내용 | 확보되는 시간 |
> |---|---|---|
> | ① 주당 시간을 올린다 | 표준 트랙을 **주 9 h**로 (23주 × 9 h = 207 h) | +23 h |
> | ② 실습을 최소 경로로 줄인다 | M16 실습을 9 h → **7 h**([§M16.L](./01_모듈/M16_DUT검증과튜닝과자동화.md)), M17 실습을 10 h → **8 h**([§M17.L](./01_모듈/M17_보드설계와인증.md)) 로 | +4 h |
> | ③ 주차를 늘린다 | 1주 배정인데 14 h 이상인 **M15·M16·M17** 에 2주씩 배정 (23 → 26주) | +24 h |
>
> **②만으로는 17 h를 못 채웁니다(+4 h).** 장비가 있는 교육이라면 ③(26주)을,
> 장비 없이 설계·계산 중심으로 진행한다면 **①+②**(+27 h)를 권합니다.
> 어느 쪽을 골랐는지 학습 계약서(1주차 산출물)에 적어 두십시오.
...
> **이 모듈이 커리큘럼의 정점(capstone of theory)입니다.** RF 시스템 엔지니어가 가장 자주 쓰는 산출물인 캐스케이드 예산표를 직접 만듭니다.
...
| **5 (완료)** | **Part III** — M06, M07 (그림 12종 포함) | 대 |
| **6 (완료)** | **Part III** — M08 (단독, 최대 밀도, 그림 10종 포함) | 대 |
| **7 (완료)** | **Part III** — M09 (그림 9종 포함) | 대 |
| **8 (완료)** | **Part IV** — M10, M11 (그림 15종 포함) | 대 |
| **9 (완료)** | **Part IV** — M12 (단독 배정, 그림 7종 포함) | 대 |
| **10 (완료)** | **Part IV** — M13 (단독, 그림 8종 포함) | 대 |
| **11 (완료)** | **Part V** — M14, M15 (그림 19종 포함) | 대 |
| **12 (완료)** | **Part V** — M16 (그림 10종 포함) | 대 |
| **13 (완료)** | **Part V** — M17 (그림 11종 포함) | 대 |
| **14 (완료)** | **Capstone** — 과제 정의 + P1~P4 (그림 4종 포함) | 중 |
| **15 (완료)** | 부록 B — 공식 치트시트 (전 항목 수치 검산) | 중 |
| **16 (완료)** | 부록 C — 출처 통합목록 (본문에서 자동 생성) | 중 |
| 17 | 전체 구조 정합성 검토 + 교재 최종 재빌드 | 중 |

각 턴의 산출물은 **P6-a의 3회 검토**를 그 턴 안에서 수행하고, 마지막 턴에서 **전체 구조 정합성 검토**를 한 번 더 수행합니다.
...
| v1.3 | 2026-08-21 | **17단계 전체 구조 정합성 검토 반영.** §4.2 주차표에 **본문 분량 · 배정 시간** 두 열을 붙이고, 모듈 본문 합계 201 h 대 배정 184 h 의 **17 h 부족**과 닫는 방법 세 가지를 명시. mermaid 라벨 '스미스차트' → '스미스 차트'. 검사 자동화: `scripts/check_structure.py`(상호 참조·링크·그림 파일·개념 소유권·그림 번호·용어 표기·주차표 대조)와 `scripts/check_abbr.py`(축약어 병기) 신설 | 검토 상한(3회) 도달로 새 검토가 아니라 **기계 검사**로 대체. 사람 판단이 필요한 항목은 손대지 않았다 |
