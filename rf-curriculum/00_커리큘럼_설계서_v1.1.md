...
flowchart TD
    ...
    CAP["<b>Capstone</b><br/>2.4 GHz 수신 프론트엔드<br/>설계 → 제작 → 측정 → 튜닝 → 리포트"]
...
| 트랙 | 대상 | 기간 | 주당 시간 | 범위 |
|---|---|---|---|---|
| **A. 집중(Intensive)** | 신입 RF 엔지니어 OJT | **12주** | 20 h (이론 8 + 실습 12) | 전 모듈 + 캡스톤 |
| **B. 표준(Standard)** | 학부생/직무 전환자 | **24주** | 8 h (이론 4 + 실습 4) | 전 모듈 + 캡스톤 |
...
### **Capstone — 2.4 GHz 수신 프론트엔드 종합 프로젝트**

| 항목 | 내용 |
|---|---|
| **선수** | M17 (전 모듈) |
| **과제** | 주어진 시스템 요구사항으로부터 2.4 GHz 대역 수신 프론트엔드를 **설계 → 예산 배분 → 부품 선정 → 보드 설계 → 측정 → 튜닝 → 판정 → 보고**까지 수행 |
| **요구사항(예시)** | 대역 2400–2483.5 MHz, 감도 −95 dBm @ 20 MHz BW, NF ≤ 4 dB, IIP3 ≥ −10 dBm, 대역외 차단 요구 있음, 소비전력 상한, ETSI EN 300 328 적용 |
| **산출물** | ① 요구사항 분석서 ② 캐스케이드 예산표 ③ 부품 선정 근거서(데이터시트 매핑) ④ 회로도 + 보드 설계 파일 ⑤ 시험 계획서 ⑥ 측정 리포트(불확도 포함) ⑦ 튜닝 기록 ⑧ 규격 적합성 판정서 ⑨ 설계 리뷰 발표자료 |
| **평가** | §10 루브릭 L1–L4로 항목별 채점. **L3 이상이 실무 투입 가능 수준** |
| **대체 과제(장비 부족 시)** | 공개 평가보드/공개 S-파라미터 데이터를 사용해 ⑤~⑧을 시뮬레이션·데이터 기반으로 수행 |
...
| 스택업·재료·Dk/Df | **M17** | M02 참조(전송선 파라미터) |
...
| **커넥터 토크 규격** | 3 | D, D, D | ⚠️ **미해결 — 등급 D만.** 값이 상충함(SMA에 4 / 5 / 8 in-lb, 재질·용도별로 다름). **집필 시 커넥터 제조사 1차 사양서 확보 필수**. 확보 실패 시 "재질·제조사별로 다르므로 반드시 해당 제조사 사양을 확인하라"로 서술하고 단일 수치를 제시하지 않는다 |
| **T1 장비 가격** | 2 | D, D | ⚠️ 시점 변동 — 집필 시 재확인, 본문에 "확인 시점" 병기 |
...
|---|---|---|
| **1 (완료)** | 본 설계서 + 리서치 | — |
| 2 | **공용 자산 선행 제작**: SVG 회로 심볼 세트, 그림 생성 스크립트 기반, 부록 A(축약어 마스터 목록) 초판, 부록 D(장비·소프트웨어 준비 가이드) | 중 |
| 3 | **Part 0 + Part I** — M00, M01, M02, M03 (그림 포함) | 대 |
| 4 | **Part II** — M04, M05 | 중 |
...
1. **모듈 순서와 범위**에 수정하고 싶은 부분이 있는지 (특히 M04/M05 조기 측정 배치, M17 보드설계 최후 배치)
2. **대상 응용 분야** — 현재 셀룰러/Wi-Fi 중심으로 설계했습니다. 레이더/위성/계측/의료 등 특정 분야에 무게를 두어야 한다면 M10·M12·M13의 사례를 교체하겠습니다
3. **캡스톤 주제** — 2.4 GHz 수신 프론트엔드가 적절한지, 아니면 송신기/트랜시버/안테나 쪽이 나은지
4. **보유 장비** — T0/T1/T2 중 어디를 기준으로 실습을 상세화할지
5. **다음 턴 진행 순서** — 위 표대로 진행할지, 특정 모듈을 먼저 볼지
...
- **지침 3(실무 수준)** — 도달 목표 G1–G11(§1.2), 전용 모듈 M14/M15/M16/M17, 3계층 장비(§9), 캡스톤 산출물 9종.
