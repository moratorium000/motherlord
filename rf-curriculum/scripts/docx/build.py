...
PARTS = [
    ("Part 0 — 출발점", "01_모듈", ["M00_RF시스템엔지니어링이란"]),
    ("Part I — RF의 공용어", "01_모듈",
     ["M01_데시벨과전력", "M02_전송선로", "M03_S파라미터와스미스차트"]),
    ("Part II — 손에 잡히는 RF", "01_모듈",
     ["M04_RF실험실입문", "M05_첫측정"]),
    ("Part III — 부품과 블록", "01_모듈",
     ["M06_수동소자와공진", "M07_필터와수동네트워크",
      "M08_증폭기", "M09_주파수변환과신호원"]),
    ("Part IV — 시스템", "01_모듈",
     ["M10_안테나와전파", "M11_트랜시버아키텍처",
    ...
    ("Part V — 측정과 검증", "01_모듈",
     ["M14_정밀측정1_교정과불확도",
      ...
      "M16_DUT검증과튜닝과자동화",
      "M17_보드설계와인증"]),
    ("Part VI — 캡스톤: 2.4 GHz 송수신 트랜시버", "02_캡스톤",
     ["00_과제정의서", "P1_시스템설계", "P2_구현설계",
      "P3_검증", "P4_튜닝판정보고"]),
]
...
저장소의 마크다운 원본을 한 권으로 묶은 것으로, **M00부터 M17까지 본문 열여덟 개
모듈, 캡스톤 과제 다섯 편, 부록 다섯 편** — 즉 커리큘럼 전편을 담았습니다.
...
| 무엇을 하려는가 | 어디로 |
|---|---|
| 처음부터 배운다 | Part 0 → Part V 를 순서대로. 각 모듈 머리의 **선수 모듈** 표를 보고 건너뛸 곳을 정하십시오 |
| 배운 것을 써 본다 | **Part VI 캡스톤** — 2.4 GHz 송수신 트랜시버를 요구사항부터 판정까지 |
| 공식이 기억나지 않는다 | **부록 B 공식 치트시트** — 커리큘럼의 모든 식을 소유 모듈·흔한 실수와 함께 |
| 약어의 뜻을 모른다 | **부록 A 축약어 마스터 목록** |
| 원문을 확인하고 싶다 | **부록 C 출처 통합목록** — 인용한 주소 전부를 신뢰 등급과 함께 |
| 장비를 갖춰야 한다 | **부록 D 장비·소프트웨어 준비 가이드** (Tier 0/1/2) |
| 수학이 막힌다 | **부록 E 수학·회로 보충** |
