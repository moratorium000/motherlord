...
GLOBS = ("01_모듈/M*.md", "02_캡스톤/*.md", "03_부록/A_*.md",
         ...
         # 심화 과정. 설계서(00_)와 캡스톤 과제문서(AC*)는 계획·과제 문서라
         # 새 출처를 들고 오지 않으므로 B01–B12 본문만 훑는다.
         "05_심화/B*.md")
    ...
    for f in files:
        ...
        for sm in re.finditer(r"^## [^\n]*출처[^\n]*$(.*?)(?=^## |\Z)",
            ...
            for ln in sm.group(1).splitlines():
                ...
                if not links:
                    continue
                # 등급 칸의 자리는 문서마다 다르다. 기본 모듈은
                # | 내용 | 출처 | 등급 | 교차검증 | 이라 셋째 칸이지만
                # 심화 모듈은 | 주장 | 등급 | 출처 | 라 둘째 칸이다.
                # 자리를 고정하지 말고 **링크가 없는 등급 모양의 칸**을 찾는다.
                gcell = ""
                for c in cells[1:]:
                    if LINK.search(c):
                        continue
